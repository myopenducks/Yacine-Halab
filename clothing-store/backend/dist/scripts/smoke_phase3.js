...
async function req(method, path, opts = {}) {
    ...
    if (opts.expectStatus != null && res.status !== opts.expectStatus) {
        throw new Error(`${method} ${path} expected ${opts.expectStatus}, got ${res.status}. Body: ${txt.slice(0, 300)}`);
    }
...
async function main() {
    ...
    const total0 = list0.json.data.total;
    ...
    log('GET /products/:id → 200', g1.json.data?.id === p1Data.id);
    // 13. get by id not found → 404
    const g2 = await req('GET', '/api/v1/products/9999999', { token: T(), expectStatus: 404 });
    log('GET /products/9999999 → 404 PRODUCT_NOT_FOUND', g2.json.error?.code === 'PRODUCT_NOT_FOUND');
    ...
    log('GET deleted product → 404', g3.json.error?.code === 'PRODUCT_NOT_FOUND');
    ...
    const errCode = del2.json.error?.code;
    const errMsg = del2.json.error?.message ?? '';
    ...
    log('DELETE nonexistent product → 404', del3.json.error?.code === 'PRODUCT_NOT_FOUND');
    ...
    log('POST invalid body → Zod 400 VALIDATION_ERROR', bad2.json.error?.code === 'VALIDATION_ERROR', `code=${bad2.json.error?.code}`);
    // 19. no token on /products → auth hook 401 (not 404 or 200)
    const noAuth = await req('GET', '/api/v1/products', { expectStatus: 401 });
    log('GET /products NO TOKEN → 401 UNAUTHORIZED', noAuth.json.error?.code === 'UNAUTHORIZED', `code=${noAuth.json.error?.code}`);
    // Summary
    console.log('\n-------');
...
//# sourceMappingURL=smoke_phase3.js.map